...
- Need Core Data with async/await or actors.
- `NSManagedObject` instances cross context/actor boundaries.
- Resolving default `@MainActor` isolation conflicts with generated NSManagedObject subclasses.

Skip if:

- General actor isolation, not Core Data specific. Use `actors.md`.
- Need general Sendable guidance. Use `sendable.md`.
...
Core Data thread safety rules unchanged with Swift Concurrency:
...
- **Sendable**: Safe across isolation domains
...
- **Forces concurrency**: Even for main thread ops
...
**Benefit**: Full isolation control.
...
2. **Manual code generation** from start
...
- **Passing `NSManagedObject` instances across actors**: Always transfer `NSManagedObjectID` or Sendable value snapshot.
- **Using `@unchecked Sendable` on `NSManagedObject`**: Not thread-safe. Object still bound to context's queue.
- **Skipping `perform { }`**: All background context access must go through `perform` or `performAndWait`.
- **Accessing `viewContext` from background task**: View context belongs to main actor; access only from `@MainActor`-isolated code.

## Further Learning

Core Data best practices, migration strategies, advanced patterns:
