...
- Need time-based operators (debounce, throttle, timers).
- Need to combine multiple async sequences (merge, combineLatest, zip).
- Migrating from Combine or RxSwift operators to Swift Concurrency equivalents.

Skip if:

- Need basic `AsyncStream` bridging for callbacks or delegates. Use `async-sequences.md`.
- Choosing between `Task`, `async let`, or task groups. Use `tasks.md`.
...
Extends Swift's AsyncSequence with time-based operators, stream combination tools, multi-consumer primitives.

**Use for**:
- Time-based ops: debounce, throttle, timers
- Combining streams: merge, combineLatest, zip, chain
- Multi-consumer: AsyncChannel for backpressure
...
Use for dependent values needing synchronization.
...
- **Manual debounce with `Task.sleep`**: Creates multiple concurrent tasks, risks out-of-order results. Use stream-based `debounce(for:)` instead.
- **Sharing `AsyncStream` across multiple consumers**: Values split unpredictably. Use `AsyncChannel` for multi-consumer with backpressure. Note: `AsyncChannel` is point-to-point, not broadcast like Combine's `.share()`.
- **Looking for `.flatMap` equivalent**: Use `TaskGroup` for fan-out; semantics differ from Combine/Rx `flatMap`.
- **Looking for `.receive(on:)` equivalent**: Use `@MainActor` or `Task` context for isolation instead.

## Best Practices

1. **Time-based operators** for rapid inputs: debounce() for search, throttle() for buttons
2. **Combine streams** with merge/combineLatest instead of manual state management
3. **AsyncChannel** for multi-consumer with backpressure
...
6. **Right tool**: AsyncAlgorithms for complex streams, AsyncStream for bridging callbacks
