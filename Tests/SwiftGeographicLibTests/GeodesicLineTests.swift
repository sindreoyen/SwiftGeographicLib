...
struct GeodesicLineTests {
    ...
    func testDirectFromPosition() {
        ...
        let azi   =  60.0
        let s12   = 5000.0
        let line  = GeodesicLine(directFrom: start, azimuth: azi, distance: s12, caps: 0)
    ...
    func testGenPositionMatchesDirect() {
        ...
        let caps: UInt32 = (1 << 11)       // GEOD_DISTANCE_IN
        | (1 << 8)       // GEOD_LONGITUDE
        | (1 << 3)       // (the extra bit GEOD_LONGITUDE carries)
        let line = GeodesicLine(from: start, azimuth: azi, caps: caps)
        
        line.setDistance(s12)
        let (lat2, lon2, _, _, _, _, _, _, _) = line.genPosition(flags: 0, s12_a12: s12)
        let direct = Geodesic.direct(from: start, distance: s12, azimuth: azi)
    ...
    func testGenSetDistance() {
        ...
        let line1 = GeodesicLine(from: start, azimuth: azi, caps: caps)
        line1.setDistance(s13)
        let pos1 = line1.position(distance: s13)
        
        let line2 = GeodesicLine(from: start, azimuth: azi, caps: caps)
        line2.genSetDistance(flags: 0, s13_a13: s13)
